...
**Context:** The ARB framework lists ten personas and gives guidance on which to select based on decision type, but provides no guidance on how many to activate for a given decision size or risk level. For a small low-risk decision, one persona may be sufficient. For a major architectural commitment, all ten may be warranted. Without a heuristic, every review defaults to judgment — which undermines consistency.
**Recommended change:** Add a decision sizing guide to `arb/how-arb-works.md` that maps decision size and risk level to a recommended number of personas — e.g. small/low-risk (1–2 personas), standard (3–5), significant/high-risk (6–10).
